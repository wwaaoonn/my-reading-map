...
エルボー法の値を軸に選び、`冊数 ÷ 5`（小数点以下切り捨て）を上限として頭を押さえます。
これはkの上限であり、各クラスタの冊数を拘束するものではありません。KMeansの割り当て
次第で5冊未満のクラスタはできます（サンプルデータ20冊ではk=4が選ばれ、内訳は
4冊・2冊・4冊・10冊です）。選んだ値と対立する候補は両方表示し、`--k` で上書きできます。
