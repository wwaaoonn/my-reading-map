...
`src/cluster.py` の `evaluate_k` と `suggest_k` が担当します。kを2から順に変えて
クラスタリングし、エルボー法（inertiaの下がり方が鈍くなる点）とシルエット係数
（クラスタの分離）を計算します。エルボー法の値を軸に選び、`冊数 ÷ 5`（小数点以下
切り捨て）を上限とします。これはkの上限で、各クラスタの冊数は拘束しません。上限は
20冊で4、50冊で10、100冊で20です。選んだ値と対立する候補は両方表示し、`--k` で
上書きできます。

シルエット係数（コサイン距離）がどのkでも0.25を下回る場合は、その旨を実行時に
表示します。0.25は「実質的な構造が見つからない」とされる慣用的な境界です。
