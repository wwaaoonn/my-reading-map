...
無ければ頻出語をつないだ名前に自動で代替します。命名のときは、クラスタごとに最大8冊分の
タイトルと説明文（先頭300文字）・頻出語・冊数をAPIに送ります。`--no-ai-names` を付けると
APIを呼ばず、処理はすべて手元で完結します。
...
2つで、3冊以上必要です。加えて、内容の異なる説明文が2件以上必要です。条件を満たさなければ
その場で止まります。
...
エルボー法の値を軸に選び、`冊数 ÷ 4`（小数点以下切り捨て）と試したkの最大値（14）の
小さいほうを上限、2を下限とします。エルボー法の値が上限を超える場合は、上限を採用します。対立する候補は両方表示します。

シルエット係数（コサイン距離）は、試した各kと採用したkについて表示します。0.25を下回る
場合は、クラスタが明確に分離していない旨をあわせて表示します。
...
割り当て後、採用したkでのシルエット係数を計算します。`--k` を指定した場合はステップ3を
...
次に、クラスタの重心に近い本の情報をClaude APIに渡し、共通するテーマを表す短い見出しを
作らせます。返り値はPydanticのモデルで構造化出力として受け取ります。

### 6. t-SNEで2次元化して描画

`src/visualize.py` が担当します。t-SNEで384次元のベクトルを2次元に落とします。距離は
コサインを使い、perplexityは冊数から決めます（`min(30, (冊数 - 1) // 3)`、下限5）。
`--perplexity` でも指定できます。図中のクラスタ名は、重なりを自動回避して配置します。

全体マップ上の楕円はクラスタの領域の目安で、統計的な信頼区間ではありません。楕円は2冊以上のクラスタに描き、2冊のクラスタや直線状に並んだクラスタでは、短軸を長軸の0.35倍まで確保します。

t-SNEの乱数は固定しています。同じCSVを同じ設定で実行すれば、座標は前回と一致します。
説明文や冊数が変わると座標は変わります。クラスタ名は実行のたびに生成AIが作るので、
同じCSVでも文言は変わります（`--no-ai-names` のときは頻出語から作るので変わりません）。
